...
"""
SDV sample 'CUJ-Rob-031' test

Verifies:
 * Service bundle observer can detect unavailability of SOME/IP publisher.
...
class SdvSampleCujRob031Test(sdv_someip_robustness_base_test.SdvSomeIpRobBaseTestClass):
    def setup_class(self):
        super().setup_class()
        self.set_sys_property(CHECKER_MODE_PROP, 0)
        self.set_sys_property(CHECKER_CUJ_PROP, 31)

        self.sdv_device2.reboot_device_and_verify_logcat()

    def test_cuj_rob_031(self):
        logging.info(
            f'{self.get_suite_name()}#{self.current_test_info.name} started'
        )
        self.verify_robustness_bundle_output(cuj_num=31)
        logging.info(
