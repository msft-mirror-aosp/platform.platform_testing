...
"""
SDV sample 'CUJ-Rob-051-IVI' test.
"""
import time
...
class SdvLongRunningCujRob051IviTest(
    sdv_ivi_rob_base_test.SdvIviRobBaseTest,
    parameterized.TestCase,
):
    """CUJ-Rob-051-IVI test."""
    ...
    def test_pubsub_after_pub_stop_core_publishes_ivi_receives(self, _number):
        self.log_enter()
        ...
        # WHEN Foo is stopped.
        self.transition_bundle_lifecycle(
            self.foo_bundle, LifecycleTransition.STOPPED
        )
    ...
    @parameterized.parameters(range(ROBUSTNESS_REPETITIONS))
    def test_pubsub_after_pub_stop_ivi_publishes_core_receives(self, _number):
        self.log_enter()
        ...
        # WHEN the IVI app (publisher) is stopped.
        self.ivi_vm_device.execute_shell_command(self.STOP_APP_CMD)

        # THEN Bar receives no new messages.
        self.verify_no_new_message_received_on_core(self.bar_bundle)

        # Cleanup publisher thread
    ...
    def teardown_test(self):
        # Teardown from SdvIviRobBaseTest does not apply here
        # since this test does not use the native DT publisher
        pass
