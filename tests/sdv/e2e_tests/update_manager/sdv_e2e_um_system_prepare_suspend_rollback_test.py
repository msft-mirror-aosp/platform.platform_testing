...
"""
SDV Update Manager Agent System Prepare Suspend Rollback Test

Check that a rollback during the PREPARE_SUSPEND_COMPLETE state for a system update succeeds.
"""
from sdv_test_fw.test_execution import sdv_base_test, sdv_test_runner
from sdv_test_fw.update.update_manager_base_class import UpdateManagerBaseClass, InterruptAction


class SdvE2EUMSystemPrepareSuspendRollbackTest(sdv_base_test.SdvBaseTestClass, UpdateManagerBaseClass):
    ...
    def test_system_prepare_suspend_rollback(self):
        ...
        rollback_output = self.client.rollback()
        self.assert_in("Cancelling prepare request", rollback_output)
        self.assert_in("Rollback started", rollback_output)
        self.assert_in("Rollback completed", rollback_output)
