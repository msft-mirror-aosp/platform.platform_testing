...
from system.software_defined_vehicle.telemetry.proto.metrics_configuration.metrics_configuration_pb2 import ConditionalTrigger, MetricsConfig, ServicePublisher
...
def make_metrics_config(
    ...
    for i, topic in enumerate(
        ...
        publisher = metrics_config.publishers.add()
        publisher.name = f'publisher_service_subscription_{i}'
        publisher.service_publisher.service_name = topic.name
        publisher.service_publisher.connection_type = (
            ServicePublisher.ConnectionType.SUBSCRIPTION
        )

        trigger = metrics_config.triggers.add()
        trigger.name = f'trigger_data_{i}'
        trigger.data_trigger.publisher_name = publisher.name
        data_trigger_names.append(trigger.name)

        expression_node = ExpressionNode()
        expression_node.field_leaf_node.publisher_name = publisher.name
    ...
    for i, topic in enumerate(
        ...
        publisher.service_publisher.service_name = topic.name
        publisher.service_publisher.connection_type = (
            ServicePublisher.ConnectionType.GETTER
        )
    ...
    for i in range(TYPICAL_METRICS_CONFIG['AGGREGATION_PUBLISHERS']):
        ...
        publisher.aggregation_publisher.reset_on_get = False
        publisher.aggregation_publisher.trigger_names.extend(
            random.choices(data_trigger_names, k=1)
        )
        publisher.aggregation_publisher.message_builder.message_type = '.Value'
        ...
        field_assignment = (
            publisher.aggregation_publisher.message_builder.field_assignments.add()
        )
        ...
        if random.choice([True, False]):
            ...
            vec_publisher = metrics_config.publishers.add()
            vec_publisher.name = f'publisher_aggregation_{i}_vec'
            vec_publisher.aggregation_publisher.reset_on_get = False

            vec_publisher.aggregation_publisher.trigger_names.extend(
                random.choices(data_trigger_names, k=1)
            )
            vec_publisher.aggregation_publisher.message_builder.message_type = (
                '.Values'
            )
            vec_field_assignment = (
                vec_publisher.aggregation_publisher.message_builder.field_assignments.add()
            )
            ...
            vec_expression_node.field_leaf_node.publisher_name = (
                vec_publisher.name
            )
