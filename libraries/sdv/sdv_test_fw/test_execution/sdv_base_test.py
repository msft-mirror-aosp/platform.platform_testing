...
class SdvBaseTestClass(base_test.BaseTestClass):
  ...
  def __get_device_list(self):
    ...
    for index in range(self.__num_of_devices):
      ...
      """
        ...
        default order.
      """
      if self.__is_device_tag_update_needed(device.info.instance_name, index + 1):
  ...
  def _debug_sigterm_handler(self, signum, frame):
    ...
    raise signals.TestAbortAll('Test aborted due to SIGTERM (timeout or external abort). Stack traces dumped above.')
  ...
  def set_devices(self, num_devices):
    ...
    It creates variables for each device in the format sdv_device{i}
  ...
  def log_test_info(self, message):
    ...
    logging.info(f'{self.get_suite_name()}#{self.current_test_info.name} :: {message}')
