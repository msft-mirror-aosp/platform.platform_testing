...
/**
 * Android Unit tests for {@link PerfettoTracingPerRunStrategy}.
 *
 * <p>To run: atest
 * CollectorDeviceLibTest:android.device.collectors.PerfettoTracingPerRunStrategyTest
 */
@RunWith(AndroidJUnit4.class)
public class PerfettoTracingPerRunStrategyTest {
    ...
    private PerfettoTracingStrategy initStrategy(Bundle b) {
        PerfettoTracingStrategy strategy =
                spy(
                        new PerfettoTracingPerRunStrategy(
                                mPerfettoHelper,
                                mInstrumentation,
                                mWakeLockContext,
                                () -> null,
    ...
    @Test
    public void testPerfettoTraceStartOnRunStart() {
        Bundle b = new Bundle();
    ...
    @Test
    public void testPerfettoTraceStartOnRunStartMultiple() {
        Bundle b = new Bundle();
        PerfettoTracingStrategy strategy = initStrategy(b);
        doReturn(true).when(mPerfettoHelper).startCollecting();
        doReturn(true).when(mPerfettoHelper).stopCollecting(anyLong(), anyString());
        ...
        strategy.testRunStart(mDataRecord, mRunDesc);
        verify(mPerfettoHelper, times(1)).startCollecting();
        strategy.testRunStart(mDataRecord, mTest1Desc);
     ...
     */
    @Test
    public void testPerfettoTraceDoesNotStartOnTestStart() {
        Bundle b = new Bundle();
        PerfettoTracingStrategy strategy = initStrategy(b);
        doReturn(true).when(mPerfettoHelper).startCollecting();
        doReturn(true).when(mPerfettoHelper).stopCollecting(anyLong(), anyString());
    ...
    /*
     * Verify perfetto does not stop collection on test end
     */
    @Test
    public void testPerfettoTraceDoesNotStopOnTestEnd() {
        Bundle b = new Bundle();
        PerfettoTracingStrategy strategy = initStrategy(b);
        doReturn(true).when(mPerfettoHelper).startCollecting();
        doReturn(true).when(mPerfettoHelper).stopCollecting(anyLong(), anyString());
