...
"""SDV Device From SDV test framework"""
...
class SdvDeviceAdb:
    ...
    def advance_logcat(self):
        ...
        Returns:
            LogcatProcessor: A log processor object for detail logcat processing.
        """
        log = self.execute_shell_command('logcat -d')
        return log_processor.LogcatProcessor(log)

    def verify_logcat_is_running(
        self, timeout=DEFAULT_TIMEOUT_LOGCAT_SECONDS
    ):
        ...
        while time.time() < end_time:
            if self.grep_from_logcat(self.LOGCAT_NON_EMPTY_LINES_GREP_TEXT, logcat_args='-v printable'):
                return
    ...
    def wait_for_device_online(self, timeout=DEFAULT_WAIT_TIMEOUT_SECONDS):
        ...
        Args:
            timeout: How long to wait for devices to be online (default 60 seconds).
    ...
    def wait_for_device_offline(self, timeout=DEFAULT_WAIT_TIMEOUT_SECONDS):
        ...
        Args:
            timeout: How long to wait for devices to be offline (default 60 seconds).
        ...
        while time.time() < end_time:
            try:
                ...
                self.log().info('Device is still responsive to ADB shell. Continuing to wait...')
            except Exception as e:
                # Exception implies the device is not reachable via shell.
                self.log().info(
                    f"Device not responsive to ADB shell (Exception: {e}). Considering it offline.")
                return
            time.sleep(0.1)
    ...
    def get_vsock_local_cid(self):
        ...
        """
        return int(self.execute_shell_command("./system_ext/bin/vsock_local_cid"))
...
class SdvDevice:
    ...
    def __init__(self, android_device):
        self.__adb = SdvDeviceAdb(android_device)
