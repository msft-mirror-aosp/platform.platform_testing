...
"""SDV CommStack Test One VM - Ping-Pong Notifications Latency."""
...
class SdvSampleCommStackOneVMPingPongTest(sdv_base_test.SdvBaseTestClass):
  ...
  def test_comm_stack_ping_pong_latency_test(self):
    ...
    for quantity in helper.QUANTITY_PING_PONG:
      for message_size in helper.MESSAGE_SIZE:
        ...
        server_cmd_tmp_file_path = self.sdv_device.execute_shell_command_in_subprocess_log(server_cmd)

        client_cmd = helper.PING_PONG_CLIENT_COMMAND.format(message_size, quantity, message_size, quantity, quantity, message_size)
        client_cmd_result = self.sdv_device.execute_shell_command(client_cmd)
        ...
        helper.wait_for_condition(lambda: self.sdv_device.read_file(server_cmd_tmp_file_path),
                                  server_expected,
