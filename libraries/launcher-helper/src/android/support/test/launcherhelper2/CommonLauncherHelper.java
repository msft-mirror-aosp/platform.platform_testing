...
/**
 * A helper class for generic launcher interactions that can be abstracted across different types of
 * launchers.
 */
public class CommonLauncherHelper {
     ...
     * @param maxAttempts
     */
    public void scrollBackToBeginning(
            UiObject2 container, Direction backDirection, int maxAttempts) {
        int attempts = 0;
    ...
    private long verifyAppStart(String packageName, long appStartTime) {
        ...
        if (packageName != null) {
            Log.w(
                    LOG_TAG,
                    String.format("No UI element with package name %s detected.", packageName));
            boolean success =
                    mDevice.wait(Until.hasObject(By.pkg(packageName).depth(0)), APP_LAUNCH_TIMEOUT);
            if (success) {
                return appStartTime;
            } else {
                return ILauncherStrategy.LAUNCH_FAILED_TIMESTAMP;
