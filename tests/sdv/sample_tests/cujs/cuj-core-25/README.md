...
This test automates the [CUJ CORE 25](/system/software_defined_vehicle/samples/cujs/cuj-core-25/README.md).
...
cvd create \
  --report_anonymous_usage_stats=y \
  --config=sdv_core_instance3 \
  --extra_kernel_cmdline=" \
    androidboot.sdv.orchestrator_config_path=/etc/orch/vm_baz_orch_config.textproto"

# User build only: Lock the VM bootloader on third VM
sdv-avb-lock 0.0.0.0:6522
```

### Mobly test execution

```bash
atest SdvSampleCujCore25Test
```

### CATBox test execution

```bash
NOTIFY_AS_NATIVE=0.0.0.0:6520,0.0.0.0:6521,0.0.0.0:6522 catbox-tradefed run commandAndExit \
