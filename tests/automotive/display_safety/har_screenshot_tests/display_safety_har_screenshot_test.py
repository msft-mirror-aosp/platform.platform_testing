...
class DisplaySafetyHarScreenshotTest(
    display_safety_test_base.DisplaySafetyBaseTestClass
):
  """
    Display Safety HAR Screenshot Test: Compares the screenshot of HAR display
    with the golden image for different vehicle states.
  ...
  def setup_test(self):
    ...
    self.golden_image_path = self.get_golden_image_path(
        ...
        pkg_name='display_safety_har_test_resources',
    )
  ...
  def test_har_display(self):
    """
      Takes a screenshot and compares it against a golden image.
    """
    logging.info(
