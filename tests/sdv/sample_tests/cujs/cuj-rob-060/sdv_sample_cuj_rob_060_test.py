...
"""
SDV sample 'CUJ-Rob-060' test.

Verifies:
 * Service bundle can be started/destroyed via LCM commands.
 * Cross-vm pub/sub stops when the subscriber is destroyed.
 * Cross-vm pub/sub resumes when the subscriber is restarted.
"""
...
class SdvSampleCujRob060Test(
    ...
    def test_subscribe_after_suscriber_kill(self, _number):
        self.log_enter()
        ...
        # AND GIVEN Baz receives messages via polling subscriber.
        self.verify_new_message_received(self.baz_bundle)

        # WHEN Baz is destroyed.
        self.transition_bundle_lifecycle(self.baz_bundle, LifecycleTransition.DESTROYED)
        ...
        # THEN Baz receives new messages via polling subscriber.
        self.verify_new_message_received(self.baz_bundle)
